...
export function createStatusHandler(deps: Dependencies) {
  return async () => {
    try {
      ...
      const response = await deps.fetcher(deps.getUrl(), { headers: { Authorization: `Bearer ${token}`, Accept: "application/json" }, signal: AbortSignal.timeout(6000), cache: "no-store" });
